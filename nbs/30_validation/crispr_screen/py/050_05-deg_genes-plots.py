...
# %% tags=[]
OUTPUT_DIR = Path(conf.RESULTS["CRISPR_ANALYSES"]["BASE_DIR"], f"analyses")
...
# %%
lv_traits = pd.read_pickle(
    Path(
        conf.RESULTS["CRISPR_ANALYSES"]["BASE_DIR"],
        "lv-gene_set_decrease",
        "traits.pkl",
    ).resolve()
)
...
# %%
sg_traits = pd.read_pickle(
    Path(
        conf.RESULTS["CRISPR_ANALYSES"]["BASE_DIR"],
        "single_gene-gene_set_decrease",
        "traits.pkl",
    ).resolve()
)
...
_lv_tmp = (
    lv_traits.head(_n_top)
    .assign(category=lv_traits.apply(_assign_category, axis=1))
    .assign(group="lv")
)
_lv_tmp["value"] = _lv_tmp["value"] / _lv_tmp["value"].max()

_sg_tmp = (
    sg_traits.head(_n_top)
    .assign(category=sg_traits.apply(_assign_category, axis=1))
    .assign(group="single-gene")
)
...
# %%
_cat_order = (
    _all_tmp.groupby(["group", "category"])["value"].max().sort_values(ascending=False)
)
