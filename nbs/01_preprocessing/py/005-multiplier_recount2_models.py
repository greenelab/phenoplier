...
multiplier_full_model = readRDS(str(
    conf.MULTIPLIER['RECOUNT2_MODEL_FILE']
))
...
assert to_scinum(_tmp.loc[('KEGG_LYSINE_DEGRADATION', '1')]['p-value']) == '8.660782e-01'
...
multiplier_model_matrix_df = pd.Series(model_metadata).reset_index().rename(columns={'index': 'parameter', 0: 'value'})
