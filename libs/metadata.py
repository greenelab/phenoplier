...
GTEX_GWAS_PHENO_INFO = pd.read_csv(
    conf.PHENOMEXCAN_SETTINGS["GTEX_GWAS_PHENO_INFO_FILE"], sep="\t", index_col="Tag"
)
...
BIOMART_GENES = pd.read_csv(
    conf.PHENOMEXCAN_SETTINGS["BIOMART_GENES_INFO_FILE"], index_col="ensembl_gene_id"
)

GENE_ID_TO_NAME_MAP = load_pickle(
    Path(
        conf.PHENOMEXCAN_SETTINGS["GENES_METADATA_DIR"],
        "genes_mapping_simplified-0.pkl",
    ).resolve()
)
...
GENE_NAME_TO_ID_MAP = load_pickle(
    Path(
        conf.PHENOMEXCAN_SETTINGS["GENES_METADATA_DIR"],
        "genes_mapping_simplified-1.pkl",
    ).resolve()
)
